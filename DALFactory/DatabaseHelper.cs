...
namespace QcAnalysis
{
    public class DatabaseHelper:IDisposable
    {
        ...
        public DatabaseHelper(ConnectionType type)
        {
            switch (type)
            {
                case ConnectionType.Sql:
                    strConnectionString = ConfigurationManager.ConnectionStrings["SqlConnectionstring"].ConnectionString;
                    objFactory = SqlClientFactory.Instance;
                    break;
                case ConnectionType.SSql:
                    strConnectionString = ConfigurationManager.ConnectionStrings["SSqlConnectionstring"].ConnectionString;
                    objFactory = SqlClientFactory.Instance;
                    break;
                case ConnectionType.InSql:
        ...
        public DatabaseHelper(bool IsOracle)
            : this(ConfigurationManager.ConnectionStrings["OracleConnectionString"].ConnectionString, Providers.Oracle)
        {
        }
        ...
        public int AddParameter(string name, DbType parmType, object value)
        {
            DbParameter p = objFactory.CreateParameter();
            p.ParameterName = name;
            p.Value = value;
        ...
        /// Ö´ÐÐ´æ´¢¹ý³Ì
        /// </summary>
        /// <param name="m_ProcName"></param>
        /// <returns></returns>
        public int ExecuteDataSetProc(string m_ProcName)
        {
            objCommand.CommandText = m_ProcName;
